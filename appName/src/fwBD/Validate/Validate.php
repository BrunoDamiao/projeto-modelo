...
// use FwBD\Validate\iValidate;
use FwBD\Validate\ValidateTrait;
...
class Validate //implements iValidate
{
    ...
    public function validateData( array $rulesModel, array $datas )
    {
        ...
        foreach ($rulesModel as $field => $rules) {
    ...
    public function setMessages(array $listParams)
    {
        ...
        $field = ucfirst( str_replace( strtolower($this->model.'_'), '', $fieldPrefix) );
    ...
    {
        $rs = $this->getUniqueDB($this->model, $fields, $value);
        if ( empty($rs) )
            return true;
